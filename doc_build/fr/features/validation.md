...
les cellules cibles invalides (voir _Validation au collage_ ci-dessous), et
le rendu canvas signale toute cellule dont la valeur actuelle échoue la
validation même si elle n'a jamais été éditée (voir _Indicateur au repos_
ci-dessous).
...
Survoler une cellule invalide au repos affiche une info-bulle — un seul
élément DOM réutilisé pour toutes les cellules (pas un par cellule
invalide, donc le coût est identique qu'il y ait une seule cellule
invalide ou des milliers), positionné automatiquement sur la cellule
survolée. rs-grid ne dessine aucun visuel par défaut : la classe est
entièrement à vous, ce qui permet de reproduire directement l'info-bulle
de daisyUI :
...
- La validation s'exécute sur les éditions utilisateur (`CommitEdit` /
  `ValidateEdit`) et sur `PasteAt` (voir _Validation au collage_
