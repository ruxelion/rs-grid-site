...
5. **Update the total** — `cache.set_total_rows(n)` when the server response
   ...
   `state.apply(GridCommand::SetTotalRowCount(n))` (see _Row-number gutter
   width_ below — this is not automatic)
...
page_size)` per the [server-pagination guide](/howto/server-pagination.md)),
