...
SetColumnValueFilter { col_key: String, values: HashSet<String> } // checklist filter
ClearColumnValueFilter { col_key: String }
ClearAllFilters
SetShowFilterRow(bool)      // show/hide the floating filter row
SetFilterRowHeight(f64)     // height in logical px, ignored if <= 0.0
```

See [Filtering](/features/filtering.md) for `FilterCondition`/`FilterOp`/`UniqueValues`.
