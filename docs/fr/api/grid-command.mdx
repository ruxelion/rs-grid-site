...
au tri/filtrage. Voir [Selection](/fr/concepts/selection#colonne-de-cases-a-cocher-selection-de-lignes)
...
Voir [Filtrage](/fr/features/filtering) pour `FilterCondition`/`FilterOp`/`UniqueValues`.
...
[Validation](/fr/features/validation) pour `ValidationRule` et
`InvalidEditMode`.
:::
...
sélection/cible — voir [Sélection et presse-papiers](/fr/features/selection-clipboard).
