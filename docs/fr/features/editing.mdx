---
title: Édition
description: Édition de cellules en ligne avec champ texte et menu déroulant.
---
...
Voir [Annuler et Rétablir](/features/undo-redo) pour plus de détails.
...
Le prédicat reçoit l'index de ligne et le [`GridModel`](/fr/api/grid-model)
complet, ce qui lui permet de lire la valeur de *n'importe quelle* colonne
...
voir [Theming](/fr/theming/css-variables)).

Voir [EditablePredicate](/fr/api/column-def#editablepredicate) pour la
référence complète du type.

## Décoration par cellule

Parfois une règle métier concerne deux colonnes et aucune des deux n'est
invalide *en isolation* — par ex. une ligne est incohérente si l'une d'une
paire fichier/libellé est remplie et l'autre vide. Aucune des deux colonnes
ne peut exprimer ça avec la [Validation](/features/validation) seule,
...
[`GridModel`](/fr/api/grid-model) complet, ce qui lui permet de lire la
...
Voir [CellDecoration](/fr/api/column-def#celldecoration) et
[CellDecorator](/fr/api/column-def#celldecorator) pour la référence
complète du type.
...
acceptée. Voir [Validation](/fr/features/validation) pour les règles
