---
title: Donnees cote serveur (PageCacheDataSource)
description: Chargement asynchrone par pages avec cache LRU et placeholders de chargement.
---
...
5. **Mettre a jour le total** — `cache.set_total_rows(n)` quand la reponse
   ...
   `state.apply(GridCommand::SetTotalRowCount(n))` (voir *Largeur de la
   gouttiere des numeros de ligne* ci-dessous — ce n'est pas automatique)
...
[guide server-pagination](/fr/howto/server-pagination)), elle est dimensionnee
