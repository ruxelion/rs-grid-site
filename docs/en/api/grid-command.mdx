...
changes. See [Selection](/concepts/selection#row-selection-checkbox-column)
for the full model.
:::
...
See [Filtering](/features/filtering) for `FilterCondition`/`FilterOp`/`UniqueValues`.
...
[Validation](/features/validation) for `ValidationRule` and
`InvalidEditMode`.
:::
...
see [Selection & Clipboard](/features/selection-clipboard).
