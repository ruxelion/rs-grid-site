...
| `filter_row_height` | `f64` | `36.0` | Height of the floating filter row (logical px) — see [Filtering](/features/filtering) |
...
| `filters` | `HashMap<String, FilterCondition>` | empty | Per-column filter conditions — see [Filtering](/features/filtering) |
| `value_filters` | `HashMap<String, HashSet<String>>` | empty | Per-column value-set restrictions (checklist filter), AND-combined with `filters` — see [Filtering](/features/filtering) |
...
| `invalid_edit_mode` | `InvalidEditMode` | `Revert` | Policy applied when a `CommitEdit` fails validation — see [Validation](/features/validation) |
...
[Validation](/features/validation) for the full behaviour.
