---
title: Filtering
description: Per-column condition filtering (contains, equals, greater than, ...) plus an AG-Grid-style value checklist, via an optional floating filter row and popup, or the programmatic API.
---
...
- "(Select All)" is a real tri-state control (checked / unchecked /
  indeterminate), acting on the currently *visible* (search-filtered)
  values when toggled.
