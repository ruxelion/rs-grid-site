...
| `--rs-grid-checked-row-bg` | `checked_row_bg` | `rgba(66,42,213,0.09)` | `rgba(99,80,240,0.12)` | Background overlay for a row checked via the [row-selection checkbox column](/concepts/selection#row-selection-checkbox-column) |
| `--rs-grid-locked-cell-bg` | `locked_cell_bg` | `rgba(0,0,0,0.04)` | `rgba(255,255,255,0.04)` | Background overlay for a non-editable cell (see [Per-cell editability](/features/editing#per-cell-editability)) |
| `--rs-grid-locked-cell-text` | `locked_cell_text` | `rgba(24,29,31,0.55)` | `rgba(208,208,208,0.55)` | Text color for a non-editable cell |
| `--rs-grid-invalid-cell-border` | `invalid_cell_border` | `rgb(239,68,68)` | `rgb(220,38,38)` | Border for a cell whose current value fails validation, shown even at rest (see [Validation](/features/validation)) |
